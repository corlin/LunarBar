...
class CalendarManager: ObservableObject {
  ...
  private func getEventsByDate(from startDate: Date, to endDate: Date) async -> [CalendarEvent] {
    ...
    let predicate = eventStore.predicateForEvents(
      withStart: startDate, end: endDate, calendars: calendarsToFetch)
  ...
  private func getFilterCalendarIds() -> Set<String>? {
    if let decodedIDs = try? JSONDecoder().decode(
      [String].self, from: SettingsManager.filterCalendar)
    {
  ...
  private func generateCalendarGrid(for date: Date, events: [Date: [CalendarEvent]]) {
    ...
    for day in gridDates {
      ...
      let ganzhiMonth = LunarDateHelper.getMonthGanZhi(
        yearGanzhi: ganzhiYear, lunarMonth: lunarMonth)
      let ganzhiDay = LunarDateHelper.getDayGanZhi(for: day)

      let monthName =
        (lunarMonth > 0 && lunarMonth <= lunarMonthSymbols.count)
        ? lunarMonthSymbols[lunarMonth - 1] : ""
      let dayName =
        (lunarDay > 0 && lunarDay <= lunarDaySymbols.count) ? lunarDaySymbols[lunarDay - 1] : ""

      let short_lunar = (lunarDay == 1) ? monthName : dayName
      let full_lunar =
        "\(ganzhiYear)年 \(ganzhiMonth)月 \(ganzhiDay)日 (\(zodiac)) \(monthName)\(dayName)"
      ...
      let holidays = HolidayHelper.getHolidays(
        date: day, lunarMonth: lunarMonth, lunarDay: lunarDay, daysInLunarMonth: daysInLunarMonth)
